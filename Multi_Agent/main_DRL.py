...
if __name__ == '__main__':
    ...
    data_2 = {'eps_history_2': [], 'scores_2': [], 'dones_2': []}
    ...
    for i in range(n_games):
        ...
        print(observation)
        print(observation2)
        while not done and not done_2:
            state = np.array([observation])
            state_2 = np.array([observation2])
            print(state)
            print(state_2)
            actions = model.predict(state)
            actions_2 = model.predict(state_2)
            action = np.argmax(actions)
            action_2 = np.argmax(actions_2)
            observation_, reward, done, info = env.step(action)
            observation2_, reward_2, done_2, info_2 = env.step_2(action_2)
