...
def plot_path(path, goal, game):
    ...
    plt.savefig('plots_DDQN/first_path_%s' % game)
    plt.clf()
    # plt.show()
...
def plot_dones(done_array, game):
    ...
    plt.savefig('plots_DDQN/first_dones_%s' % str(game))
    plt.clf()
...
def plot_learning(scores, epsilons):
    ...
    plt.savefig('plots_DDQN/learning_%s' % str(len(scores)))
    plt.clf()
...
def plot_success_rate(dones, game):
    ...
    plt.savefig('plots_DDQN/Success_rate_%s' % str(game))
    plt.clf()
...
if __name__ == '__main__':
    ...
    agent = Agent(gamma=0.99, epsilon=0.01, lr=lr, input_dims=env.observation_space.shape,
                  ...
                  epsilon_dec=0.99, epsilon_end=0.01, fname='_New_R_target.h5')
    scores = []
    eps_history = []
    dones = []
    data = {'eps_history': [], 'scores': [], 'dones': []}
    #with open('data_DDQN.json') as json_data:
    ...
    #---------------------------------------evaluate main------------------------------------------
    #'''
    model = load_model('models_DDQN/3000_New_R_target.h5')
    path_array = []
    goals = []
    ...
    #-------------------------------Training main--------------------------------------------
'''
    latest = tf.train.latest_checkpoint('checkpoints_DDQN')
    print(latest)
    agent.load_weights(latest)
    model = load_model('models_DDQN/1100_New_R_target.h5')
    agent.q_eval = model
    agent.target_net = model

    #for i in range(n_games):
    for i in range(1100,n_games+1):
        agent.alternate_model=agent.q_eval
        ...
        while not done:
            ...
            print('lerning rate is : %s' % agent.target_net.optimizer.get_config())
            print('-'*79)
        ...
        data['scores'] = scores
        data['dones'] = dones
        with open('data_DDQN.json', 'w', encoding='utf-8') as f:
