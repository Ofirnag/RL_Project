...
class Agent:
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size,
        ...
        self.alternate_model = self.q_eval
    ...
    def learn(self):
        ...
        q_alternate= self.alternate_model.predict(states_)[np.argmax(q_next,axis=1)]
        q_target = np.copy(q_eval)
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        q_target[batch_index, actions] = rewards + self.gamma *np.max(q_alternate, axis=1)*dones
